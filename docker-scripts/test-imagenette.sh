...
pgrep -f cassandra || /cassandra/bin/cassandra 2>&1 | grep "state jump to NORMAL"
    ...
    /cassandra/bin/cqlsh --ssl --cqlshrc /cassandra/conf/cqlsh.rc -e "DROP KEYSPACE IF EXISTS imagenette;"
    /cassandra/bin/cqlsh --ssl --cqlshrc /cassandra/conf/cqlsh.rc -f create_tables.cql
