...
def get_raw_image_dataset(dataset_id, bs=1024, prefetch=8, shuffle_batches=16):
...
def scan(*, tfr=False, epochs=4, bs=1024, shuffle_batches=16, log_fn=None):
    ...
    dataset = get_raw_image_dataset(dataset_id, bs=bs, prefetch=8, shuffle_batches=shuffle_batches)
    #steps = tf.data.experimental.cardinality(dataset).numpy()
    imagenet_train_set_images = 1281167
    steps = int(np.ceil(imagenet_train_set_images / bs))
