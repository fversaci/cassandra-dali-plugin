...
void BatchLoaderOOO::load_trusted_cert_file(std::string file, CassSsl* ssl) {
  CassError rc;
  char* cert;
  int64_t cert_size;
...
void BatchLoaderOOO::set_ssl(CassCluster* cluster, std::string ssl_certificate) {
...
BatchLoaderOOO::BatchLoaderOOO(std::string table, std::string label_type,
...
void BatchLoaderOOO::allocTens(int wb) {
  shapes[wb].clear();
...
void BatchLoaderOOO::copy_data_none(const CassResult* result,
...
void BatchLoaderOOO::copy_data_int(const CassResult* result,
                              const cass_byte_t* data, size_t sz,
...
void BatchLoaderOOO::copy_data_img(const CassResult* result,
                                const cass_byte_t* data, size_t sz,
...
void BatchLoaderOOO::transfer2copy(CassFuture* query_future, int wb, int i) {
  ...
  if (label_t == lab_none) {
    // enqueue image copy
    cj = copy_pool->enqueue(&BatchLoaderOOO::copy_data_none, this,
                            result, data, sz, i, wb);
    ...
    cj = copy_pool->enqueue(&BatchLoaderOOO::copy_data_int, this,
    ...
    cj = copy_pool->enqueue(&BatchLoaderOOO::copy_data_img, this,
...
std::future<BatchImgLab> BatchLoaderOOO::start_transfers(
...
BatchImgLab BatchLoaderOOO::wait4images(int wb) {
...
void BatchLoaderOOO::check_connection() {
  if (!connected) {
    connect();
    connected = true;
  }
...
void BatchLoaderOOO::prefetch_batch(const std::vector<CassUuid>& ks) {
...
BatchImgLab BatchLoaderOOO::blocking_get_batch() {
  // recover
...
void BatchLoaderOOO::ignore_batch() {
  if (!connected) {
    return;
  }
